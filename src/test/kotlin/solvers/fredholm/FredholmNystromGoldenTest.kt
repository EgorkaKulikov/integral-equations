...
class FredholmNystromGoldenTest {
    ...
    @Test fun nystrom_f2_basis_b() {
        val (s8, g8) = solver(FredholmProblem.F2, 8)
        val (s16, g16) = solver(FredholmProblem.F2, 16)
        val e8 = errorEh({ t -> FredholmProblem.F2.exact(t) }, s8.nystrom().eval, g8)
        val e16 = errorEh({ t -> FredholmProblem.F2.exact(t) }, s16.nystrom().eval, g16)
    ...
    @Test fun iterated_nystrom_f2_basis_b() {
        val (s8, g8) = solver(FredholmProblem.F2, 8)
        val (s16, g16) = solver(FredholmProblem.F2, 16)
        val e8 = errorEh({ t -> FredholmProblem.F2.exact(t) }, s8.iteratedNystrom().eval, g8)
        val e16 = errorEh({ t -> FredholmProblem.F2.exact(t) }, s16.iteratedNystrom().eval, g16)
    ...
    @Test fun nystrom_xitilde_f2_converges() {
        for (r in 1..2) {
            ...
            val (s16, g16) = xiTildeSolver(FredholmProblem.F2, 16, r)
            val e8 = errorEh({ t -> FredholmProblem.F2.exact(t) }, s8.nystrom().eval, g8)
            val e16 = errorEh({ t -> FredholmProblem.F2.exact(t) }, s16.nystrom().eval, g16)
