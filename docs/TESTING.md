...
```bash
./gradlew fastTest                  # быстрый набор: 130 тестов, единицы секунд
...
130 быстрых тестов этого репозитория (ещё 113 в `numerical-core` и 114 в
...
Все шаги передают `-PmachineDependentGates=false` (см. раздел выше). Каждый job
начинается с публикации библиотек в `mavenLocal` из их репозиториев (см. раздел
«Зависимости-артефакты»).

| Job | Что гоняет |
|---|---|
| `fast` | публикация библиотек в `mavenLocal`, `fastTest --tests solvers.core.ActiveBackendDiagnosticTest` (диагностика бэкенда), затем `fastTest koverXmlReport compileDemoKotlin compileProblemsKotlin verifyArtifactDependencies` |
...
| `full` | `slowTest` без машинно-зависимых тестов, ПОКЛАССНЫМИ шагами (см. ниже): сверка с публикацией (613 значений), перекрёстная согласованность, аналитика, порядки сходимости (`timeout-minutes: 45`) |

### Почему job `full` разбит на поклассные шаги
...
упавших тестов. При едином шаге на весь набор падение не локализуется никак.

Имя упавшего **шага** видно в API всегда и без привилегий, поэтому разбиение
переводит диагностируемость из разряда «привилегия владельца» в разряд свойств
конфигурации. Шаги после первого помечены `if: ${{ !cancelled() }}`, чтобы падение
одного класса не скрывало состояние остальных: за один прогон видна вся картина.
Цена — четыре старта демона Gradle вместо одного (~+3 мин).

Три детали, важные при правках CI:

1. Job `fast` начинается с отдельного прогона `solvers.core.ActiveBackendDiagnosticTest`
   (утверждает, что активен `MultikCpuBackend`; заменил `BackendSpiTest`, ушедший в
   `numerical-core`) — без него молчаливый откат с multik на `ReferenceBackend` выглядел
   бы как «рефакторинг испортил числа».
