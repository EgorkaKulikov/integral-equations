...
> **Точка расширения под GPU.** Бэкенд инкапсулирован в `LinearAlgebra`, поэтому
> подключение GPU-движка multik не затрагивает код решателей.
...
- [ ] GPU-движок multik для тяжёлых ядер (точка расширения уже заложена).
